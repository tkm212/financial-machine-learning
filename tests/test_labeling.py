...
def ohlc_df() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "open": [100.0] * 20,
        "high": [100.5, 101.0, 102.0, 102.5, 103.0, 99.0, 98.5, 98.0, 100.0, 100.2]
        + [100.3] * 10,
        "low": [99.5, 99.8, 100.5, 101.5, 102.0, 98.5, 97.5, 97.0, 99.5, 99.8]
        + [100.0] * 10,
        "close": [100.0, 100.5, 101.5, 102.0, 102.5, 98.5, 98.0, 97.5, 100.0, 100.0]
        + [100.2] * 10,
    })
...
def test_triple_barrier_labels_custom_columns() -> None:
    ...
    assert len(out) == 1
    assert out["label"].iloc[0] == 1  # h=101 >= 100.5*1.02=102.51? No. h goes to 105. At bar 1 h=101, at bar 2 h=102... 100.5*1.02=102.51, so bar 3 h=103 hits it. Yes.